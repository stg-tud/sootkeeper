...
public class TransitiveNativeUsageService extends AbstractSootAnalysis<TransitiveNativeResult, SootBundleConfig> {
    private static final String NAME = "peaks-transitive-native";

    public TransitiveNativeUsageService(BundleContext context) throws IllegalClassFormatException {
        super(context);
    }
    ...
    @Override
    protected TransitiveNativeResult runSootBasedAnalysis(SootBundleConfig config, Map<Class<? extends AbstractAnalysisService<? extends IAnalysisResult, ? extends IAnalysisConfig>>, IAnalysisResult> map, SootResult sootResult) {
        TransitiveNativeUsage analysis = new TransitiveNativeUsage();
        AnalysisResult result = analysis.performAnalysis(sootResult.getScene().getCallGraph());
        return new TransitiveNativeResult(result);
    }
