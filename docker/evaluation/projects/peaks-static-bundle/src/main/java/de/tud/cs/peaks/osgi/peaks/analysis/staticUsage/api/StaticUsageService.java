...
public class StaticUsageService extends AbstractSootAnalysis<StaticUsageResult, SootBundleConfig> {
    private static final String NAME = "peaks-static";

    public StaticUsageService(BundleContext context) throws IllegalClassFormatException {
        super(context);
    }
    ...
    @Override
    protected StaticUsageResult runSootBasedAnalysis(SootBundleConfig config, Map<Class<? extends AbstractAnalysisService<? extends IAnalysisResult, ? extends IAnalysisConfig>>, IAnalysisResult> map, SootResult sootResult) {
        StaticUsageAnalysis analysis = new StaticUsageAnalysis();
        CallGraph cg = sootResult.getScene().getCallGraph();
        AnalysisResult result = analysis.performAnalysis(cg);
        return new StaticUsageResult(result);
    }
