...
public class InfoflowAndroidConfiguration extends InfoflowConfiguration {
	...
	public boolean equals(Object o) {
		...
		InfoflowAndroidConfiguration that = (InfoflowAndroidConfiguration) o;

		if (computeResultPaths != that.computeResultPaths) return false;
		if (enableCallbacks != that.enableCallbacks) return false;
		if (enableCallbackSources != that.enableCallbackSources) return false;
		if (pathBuilder != that.pathBuilder) return false;
		if (callbackAnalyzer != that.callbackAnalyzer) return false;
		return layoutMatchingMode == that.layoutMatchingMode;
	}

	@Override
	public int hashCode() {
		int result = (computeResultPaths ? 1 : 0);
		result = 31 * result + (pathBuilder != null ? pathBuilder.hashCode() : 0);
		result = 31 * result + (enableCallbacks ? 1 : 0);
		result = 31 * result + (enableCallbackSources ? 1 : 0);
		result = 31 * result + (callbackAnalyzer != null ? callbackAnalyzer.hashCode() : 0);
		result = 31 * result + (layoutMatchingMode != null ? layoutMatchingMode.hashCode() : 0);
		return result;
	}
}
